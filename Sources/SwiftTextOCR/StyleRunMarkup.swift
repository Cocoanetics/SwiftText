...
extension Array where Element == StyleRun {
	...
	func inlineMarkup(suppressingUniformBold: Bool = false) -> [InlineMarkup] {
		...
		let suppressBold = suppressingUniformBold
		...
		for run in coalesced() {
			...
			let style = TextStyle(
				...
				isItalic: originalStyle.isItalic,
				isMonospaced: originalStyle.isMonospaced)
